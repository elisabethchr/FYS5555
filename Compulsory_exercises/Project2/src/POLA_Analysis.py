...
g_RawRate.Draw("AP")
C.Draw()
C.Print("../data/plots/%s/RawRate_%s.pdf" %(indir.split('/')[-1], indir.split('/')[-1]))
...
g_pressure.Draw("AP")
C1.Draw()
C1.Print("../data/plots/%s/pressure_%s.pdf" %(indir.split('/')[-1], indir.split('/')[-1]))
...
h_div_temp.Draw("e")
C3.Draw()
C3.Print("../data/plots/%s/IndoorTemp_EventsOverTime_%s.pdf" %(indir.split('/')[-1], indir.split('/')[-1]))
...
C4.Draw()
C4.Print("../data/plots/%s/IndoorTemp_%s.pdf" %(indir.split('/')[-1], indir.split('/')[-1]))

C4 = ROOT.TCanvas("c4", "Outdoor temperature over time", 1200, 600)
...
C4.Draw()
C4.Print("../data/plots/%s/OutdoorTemp_%s.pdf" %(indir.split('/')[-1], indir.split('/')[-1]))
"""
